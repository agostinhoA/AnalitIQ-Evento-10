...
<dl class="report-range"><div><dt>Fecha desde</dt><dd><c:choose><c:when test="${empty informe.rango.desde}">Sin límite</c:when><c:otherwise><fmt:formatDate value="${informe.rango.desdeFecha}" pattern="dd/MM/yyyy"/></c:otherwise></c:choose></dd></div><div><dt>Fecha hasta</dt><dd><c:choose><c:when test="${empty informe.rango.hasta}">Sin límite</c:when><c:otherwise><fmt:formatDate value="${informe.rango.hastaFecha}" pattern="dd/MM/yyyy"/></c:otherwise></c:choose></dd></div></dl>
</section>
...
<dl class="debt-summary"><div><dt>Cantidad de cuotas pendientes</dt><dd data-cantidad="<c:out value='${bloque.cantidad}'/>"><c:out value="${bloque.cantidad}"/></dd></div><div><dt>Total de deuda del tratamiento según los filtros</dt><dd class="numeric" data-total="<c:out value='${bloque.total}'/>"><fmt:formatNumber value="${bloque.total}" minFractionDigits="2" maxFractionDigits="2"/></dd></div></dl>
