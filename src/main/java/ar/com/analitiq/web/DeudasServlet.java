...
public final class DeudasServlet extends HttpServlet {
    ...
    @Override protected void doGet(HttpServletRequest req,HttpServletResponse res) throws IOException,ServletException {
        ...
        try {
            ...
            if(flujo.getPacientes().isEmpty()) {
                req.setAttribute("mensaje","No se encontró un paciente con los datos ingresados.");
    ...
    @Override protected void doPost(HttpServletRequest req,HttpServletResponse res) throws IOException,ServletException {
        ...
        try {
            ...
            if("buscar".equals(action)) {
                RangoFechas rango=RangoFechas.parse(req.getParameter("desde"),req.getParameter("hasta"));
                CriterioBusqueda criterio=CriterioBusqueda.validar(req.getParameter("modo"),req.getParameter("dni"),req.getParameter("nombre"),req.getParameter("apellido"));
                DeudasService service=obtenerServicio(req,res);
                if(service==null) return;
                id=busquedas.agregar(service.buscar(criterio),rango,Instant.now());
                ...
                RangoFechas rango=RangoFechas.parse(req.getParameter("desde"),req.getParameter("hasta"));
                id=busquedas.filtrar(req.getParameter("busqueda"),rango,Instant.now());
            ...
            if("buscar".equals(action)) vista(req,res,"buscar");
                ...
                try {
                    ...
                    if("filtrar".equals(action) && flujo.getDni()!=null) {
                        valoresEnviados(req);
                        req.setAttribute("mensaje",e.getMessage()+" No se aplicó el filtro; se mantiene el rango del informe mostrado.");
    ...
    private void mostrarInforme(HttpServletRequest req,HttpServletResponse res,BusquedaSesion.Flujo flujo)
        ...
        DeudasService service=obtenerServicio(req,res);
        if(service==null) return;
        req.setAttribute("informe",service.consultar(flujo.getDni(),flujo.getRango()));
