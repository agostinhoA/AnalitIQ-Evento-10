...
public final class PacienteDao {
    public List<Paciente> buscarConDeudas(Connection c,CriterioBusqueda criterio,FiltrosDeuda filtros) throws SQLException {
        String sql="""
            SELECT p.dni_paciente,p.nombre_paciente,p.apellido_paciente
            FROM pacientes p
            WHERE
            """;
        if(!criterio.esListado()) sql+=criterio.dni()!=null ? " p.dni_paciente=? AND " : " p.nombre_paciente=? AND p.apellido_paciente=? AND ";
        sql+="""
            EXISTS (
                ...
                WHERE t.dni_paciente=p.dni_paciente AND t.activo='SI' AND ec.tipo_estado='Adeuda'
        ...
        sql+=" ) ORDER BY p.apellido_paciente,p.nombre_paciente,p.dni_paciente";
        try(PreparedStatement s=c.prepareStatement(sql)) {
            ...
            s.setDate(index++,filtros.getRango().getDesdeFecha());
            s.setDate(index++,filtros.getRango().getHastaFecha());
